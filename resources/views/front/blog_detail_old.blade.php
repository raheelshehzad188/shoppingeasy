                        ...
                        <div class="blog__details--wrapper">
                            <div class="entry__blog">
                                <div class="blog__post--header mb-30">
                                    <h2 class="post__header--title mb-15">{{$v->title_english}}</h2>
                                    <p class="blog__post--meta">Posted by : admin / On : {{date(" F d Y ",strtotime($v->created_at))}} </p>                                     
                                </div>
                                <div class="blog__thumbnail mb-30">
                                    <img class="blog__thumbnail--img border-radius-10" src="{{asset($v->image)}}" alt="blog-img">
                                </div>
                                <div class="blog__details--content">
                                    <?php echo $v->description_english ?>
                                 </div>
                            </div>
                                    ...
                                    <div class="col mb-28">
                                        <div class="related__post--items">
                                            <div class="related__post--thumb border-radius-10 mb-15">
                                                <a class="display-block" href="blog/{{$b->slug}}"><img class="related__post--img display-block border-radius-10" src="{{asset($b->image)}}" alt="related-post"></a>
                                            </div>
                                            <div class="related__post--text">
                                                <h3 class="related__post--title"><a class="related__post--title__link" href="blog/{{$b->slug}}">{{$b->title_english}}</a></h3>
                                                <span class="related__post--deta">{{date(" F d Y ",strtotime($b->created_at))}}</span>
                                            </div>
                                        </div>
                                    </div>
                                    @endforeach
                                    ...
                                    <div class="reviews__comment--inner">
                                        ...
                                        <div class="reviews__comment--list d-flex">
                                            <div class="reviews__comment--thumb">
                                                <img class="display-block" src="{{asset('')}}front/images/user.png" alt="comment-thumb">
                                            </div>
                                            <div class="reviews__comment--content ">
                                                <div class="comment__content--topbar d-flex justify-content-between">
                                                    <div class="comment__content--topbar__left">
                                                        <h4 class="reviews__comment--content__title2">{{$c->name}}</h4>
                                                        <span class="reviews__comment--content__date2">{{date(" F d Y ",strtotime($c->created_at))}}</span>
                                                    </div>
                                                    <!--<button class="comment__reply--btn primary__btn" type="submit">Reply</button>-->
                                                </div>
                                                <p class="reviews__comment--content__desc">{{$c->comment}}</p>
                                            </div>
                                        </div>
                                        @endforeach
                                    </div>
                                    ...
                                    <form action="/blod_comment" method="post">
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-12 mb-15">
                                                <textarea class="reviews__comment--reply__textarea" name="cum" placeholder="Your Comments...." required></textarea>
                                            </div> 
                            ...
                            <div class="single__widget widget__bg">
                                ...
                                @foreach($bcate as $cate)
                                    <li class="widget__categories--menu__list">    
                                        <span class="product__items--content__title h4"><a href="/blog_category/{{$cate->slug}}">{{$cate->title_english}}</a></span>
                                    </li>
                                @endforeach
                                <!--</ul>-->
                            </div>
                            <div class="single__widget widget__bg">
                               <h2 class="widget__title h3">Recent Article</h2>
                                <div class="product__grid--inner">
                                    ...
                                    <div class="product__items product__items--grid d-flex align-items-center">
                                        <div class="product__items--grid__thumbnail position__relative">
                                            <a class="product__items--link" href="/blog/{{$p->slug}}">
                                                <img class="product__grid--items__img product__primary--img" src="{{asset($p->image)}}" alt="product-img">
                                                <img class="product__grid--items__img product__secondary--img" src="{{asset($p->image)}}" alt="product-img">
                                            </a>
                                        </div>
                                        <div class="product__items--grid__content">
                                            <h3 class="product__items--content__title h4"><a href="/blog/{{$p->slug}}">{{$p->title_english}}</a></h3>
                                            <span class="meta__deta">{{date(" F d Y ",strtotime($p->created_at))}}</span>
                                        </div>
                                    </div>
                                    @endforeach
                                </div>
